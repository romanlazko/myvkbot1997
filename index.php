...
if($type == 'message_new'){
    ...
    if($text =='Начать') {
        ...
        if($setdisen==='1'){
            ...
            $keyboard = [ 
                'one_time' => true, 
                'buttons' => keyboard("1",iconv( 'cp1251','utf-8' , 'Начать'),'positive')
            ];

            if(strpos($urlHeaders[0], '200')) {
                $reply = $first_name.iconv( 'cp1251','utf-8' ,', на имя : ').$text.iconv( 'cp1251','utf-8' ,' обнаружено письмо.
...
').$url1;
                ...
                $reply = $first_name.iconv( 'cp1251','utf-8' ,', на имя : ').$text.iconv( 'cp1251','utf-8' ,' письма не обнаружено');
                sendKeyboard($token,$user_id,$reply,$keyboard);
            }
        }
        elseif($setdisen==='2'){
            $visa_url=select_file();
            $handle = fopen($visa_url, "r");
            //sendMessage($token,$user_id,$visa_url);
            //if (($handle = fopen($visa_url, "r")) !== FALSE) {

                while (($visanum = fgetcsv($handle, 1000, ",")) !== FALSE) {
                     $cutvisa = substr($visanum[1], 2);
                     $cutvisa1 = substr($cutvisa, 0, -2);
                     if($cutvisa1===$text){
                          sendMessage($token,$user_id,$cutvisa1.' first method');
                          break;
                     }
                     elseif(strripos($visanum[1], $text)){
                          sendMessage($token,$user_id,iconv( 'ISO-8859-2','utf-8' ,$visanum[1]).' second method');
                          break;
                     }
